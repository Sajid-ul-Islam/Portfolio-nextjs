...
export default function VSCodeIDE({ children }: { children: React.ReactNode }) {
  const [activeSide, setActiveSide] = useState<"explorer" | "search" | "git" | "account" | "settings" | "terminal" | "chat">('explorer');
  const [activeFile, setActiveFile] = useState('about');
  const [showTerminal, setShowTerminal] = useState(false);
  const [showAIChat, setShowAIChat] = useState(false);

  const handleSidebarChange = (id: "explorer" | "search" | "git" | "account" | "settings" | "terminal" | "chat") => {
    if (id === 'terminal') {
