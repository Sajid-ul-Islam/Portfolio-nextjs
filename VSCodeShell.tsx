...
import AIChat from "./AIChat"; // AI Chat will be conditionally rendered
import AIChatTrigger from "./AIChatTrigger"; // AI Chat Trigger will be conditionally rendered
import IntroAnimation from "./IntroAnimation";
import BottomNavBar from "../BottomNavBar"; // New component for mobile navigation
...
const mobileItems = [
  ...
  { id: "chat", icon: "comment", label: "AI Chat" },
] as const;
...
//       return;
//     } // AI Chat will be triggered differently
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 overflow-hidden relative">
          ...
          <div
            className={cn(
              "fixed bottom-0 left-0 top-0 z-[101] w-[80vw] bg-[var(--vscode-sideBar-background)] transition-transform duration-300 ease-out",
              mobileDrawerOpen ? "translate-x-0" : "-translate-x-full"
            )}
          >
            <Sidebar isOpen={mobileDrawerOpen} activePanel={activeActivity} />
          </div>
        </div>
        <BottomNavBar
          items={mobileItems as any}
          activeItem={activeActivity}
          onItemClick={handleActivityClick as any}
          onChatClick={() => setShowAIChat(true)}
          showChat={showAIChat}
        />
        {showAIChat && (
          <div className="fixed bottom-16 left-0 right-0 h-1/2 z-[2000] animate-in slide-in-from-bottom-8 zoom-in-95 duration-500 ease-out">
  ...
  return (
    ...
    >
      ...
      {showAIChat && (
          <div className="fixed bottom-6 left-0 right-0 h-64 z-[1000]">
              <ErrorBoundary fallbackMessage="AI Neural Link disconnected. Verify environment API keys.">
                <AIChat onClose={() => setShowAIChat(false)} />
              </ErrorBoundary>
          </div>
